...
public class RequestFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException, ExpiredJwtException {
		...
		logger.warn(String.format("URI %s language %s deviceType %s role %s",request.getRequestURI().toString(), request.getHeader("Accept-Language"),request.getHeader("deviceType"),request.getHeader("role")));
		
		if(request.getHeader("role")==null) logger.warn("role header is missing in header");
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			//to allow same email/number at different roles
			UserDetails userDetails = userService.loadUserByUsername(username);
			
			if(userDetails.isEnabled()==false) {
				request.setAttribute("status",HttpStatus.FORBIDDEN.value());
				request.setAttribute("message", messages.get("user.suspended"));
